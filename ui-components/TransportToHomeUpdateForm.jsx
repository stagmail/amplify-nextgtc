...
import { getTransportToHome } from "./graphql/queries";
import { updateTransportToHome } from "./graphql/mutations";
const client = generateClient();
export default function TransportToHomeUpdateForm(props) {
  const {
    id: idProp,
    transportToHome: transportToHomeModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = transportToHomeRecord
      ? { ...initialValues, ...transportToHomeRecord }
      : initialValues;
    setPickupLocation(cleanValues.pickupLocation);
    setDropoffLocation(cleanValues.dropoffLocation);
    setPickupTime(cleanValues.pickupTime);
    setPaxNameId(cleanValues.paxNameId);
    setErrors({});
  };
  const [transportToHomeRecord, setTransportToHomeRecord] = React.useState(
    transportToHomeModelProp
  );
  React.useEffect(() => {
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getTransportToHome.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getTransportToHome
        : transportToHomeModelProp;
      setTransportToHomeRecord(record);
    };
    queryData();
  }, [idProp, transportToHomeModelProp]);
  React.useEffect(resetStateValues, [transportToHomeRecord]);
  const validations = {
    pickupLocation: [],
    dropoffLocation: [{ type: "Required" }],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateTransportToHome.replaceAll("__typename", ""),
            variables: {
              input: {
                id: transportToHomeRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "TransportToHomeUpdateForm")}
      {...rest}
    >
      <SelectField
      ...
      >
        <option
          ...
          value="LocationOne"
          {...getOverrideProps(overrides, "pickupLocationoption0")}
        ></option>
        <option
          children="Location two"
          value="LocationTwo"
          {...getOverrideProps(overrides, "pickupLocationoption1")}
        ></option>
        <option
          children="Location three"
          value="LocationThree"
          {...getOverrideProps(overrides, "pickupLocationoption2")}
        ></option>
        <option
          children="Location four"
          value="LocationFour"
          {...getOverrideProps(overrides, "pickupLocationoption3")}
        ></option>
        <option
          children="Location five"
          value="LocationFive"
          {...getOverrideProps(overrides, "pickupLocationoption4")}
        ></option>
      </SelectField>
      <TextField
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || transportToHomeModelProp)}
