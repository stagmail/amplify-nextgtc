...
import { getTransportToWork } from "./graphql/queries";
import { updateTransportToWork } from "./graphql/mutations";
const client = generateClient();
export default function TransportToWorkUpdateForm(props) {
  const {
    id: idProp,
    transportToWork: transportToWorkModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = transportToWorkRecord
      ? { ...initialValues, ...transportToWorkRecord }
      : initialValues;
  ...
  const [transportToWorkRecord, setTransportToWorkRecord] = React.useState(
    transportToWorkModelProp
  );
  React.useEffect(() => {
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getTransportToWork.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getTransportToWork
        : transportToWorkModelProp;
      setTransportToWorkRecord(record);
    };
    queryData();
  }, [idProp, transportToWorkModelProp]);
  React.useEffect(resetStateValues, [transportToWorkRecord]);
  const validations = {
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateTransportToWork.replaceAll("__typename", ""),
            variables: {
              input: {
                id: transportToWorkRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "TransportToWorkUpdateForm")}
      {...rest}
    >
      <TextField
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || transportToWorkModelProp)}
